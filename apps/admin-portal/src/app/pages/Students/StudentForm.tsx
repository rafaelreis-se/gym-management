...
import { AgeCategory, StudentStatus, GuardianRelationship } from '@gym-management/types';
...
export const StudentFormPage: React.FC = () => {
  ...
  const [existingGuardian, setExistingGuardian] = useState<Guardian | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isChild && !existingGuardian && !showGuardianForm) {
        setError('Children require a guardian. Please search for or add a guardian.');
        setLoading(false);
        return;
      }
      ...
      if (isChild && (existingGuardian || showGuardianForm)) {
        ...
        requestData = {
          ...
          guardian: existingGuardian || {
            ...guardianData,
            birthDate: guardianData.birthDate ? new Date(guardianData.birthDate) : undefined,
          },
      ...
      setError(err.response?.data?.message || 'Error saving student. Please try again.');
    } finally {
      setLoading(false);
    }
  };
                  ...
                  {!existingGuardian && !showGuardianForm && (
                    <Box>
                      <Alert severity="info" sx={{ mb: 3 }}>
                        First, search if the guardian is already registered. If not found, you can register a new one.
                      </Alert>
                      ...
                      <Grid container spacing={2} alignItems="center">
                        ...
                        <Grid item xs={12} md={4}>
                          <Button
                            fullWidth
                            variant="contained"
                            startIcon={searchingGuardian ? <CircularProgress size={20} /> : <Search />}
                            onClick={searchGuardianByCpf}
                            disabled={searchingGuardian || guardianCpf.length < 11}
                            size="large"
                          >
                            {searchingGuardian ? 'Searching...' : 'Search Guardian'}
                          </Button>
                        </Grid>
                      </Grid>
                    </Box>
                  ...
                  {existingGuardian && (
                    <Box>
                      <Alert
                        ...
                        sx={{ mb: 2 }}
                      >
                        Guardian found! The student will be linked to this guardian.
                      </Alert>
                      ...
                      <Card variant="outlined">
                        <CardContent>
                          <Box display="flex" justifyContent="space-between" alignItems="start" mb={2}>
                            ...
                            <Chip label="Existing Guardian" color="success" size="small" />
                          </Box>
                          ...
                          <Grid container spacing={2}>
                            <Grid item xs={12} sm={6}>
                              ...
                              <Typography variant="body1">{existingGuardian.email}</Typography>
                            </Grid>
                            <Grid item xs={12} sm={6}>
                              <Typography variant="body2" color="text.secondary">
                                Phone
                              </Typography>
                              <Typography variant="body1">{existingGuardian.phone}</Typography>
                            </Grid>
                            {existingGuardian.address && (
                              <Grid item xs={12}>
                                ...
                                <Typography variant="body1">
                                  {existingGuardian.address}, {existingGuardian.city} - {existingGuardian.state}
                                </Typography>
                              </Grid>
                            )}
                          </Grid>
                          ...
                          <Box mt={3}>
                            <FormControl fullWidth>
                              ...
                              <Select
                                ...
                                label="Relationship *"
                              >
                                <MenuItem value={GuardianRelationship.MOTHER}>Mother</MenuItem>
                                <MenuItem value={GuardianRelationship.FATHER}>Father</MenuItem>
                                <MenuItem value={GuardianRelationship.GRANDMOTHER}>Grandmother</MenuItem>
                                <MenuItem value={GuardianRelationship.GRANDFATHER}>Grandfather</MenuItem>
                                <MenuItem value={GuardianRelationship.AUNT}>Aunt</MenuItem>
                                <MenuItem value={GuardianRelationship.UNCLE}>Uncle</MenuItem>
                                <MenuItem value={GuardianRelationship.LEGAL_GUARDIAN}>Legal Guardian</MenuItem>
                                <MenuItem value={GuardianRelationship.OTHER}>Other</MenuItem>
                              </Select>
                            </FormControl>
                          </Box>
                        </CardContent>
                  ...
                  {showGuardianForm && (
                    <Box>
                      <Alert severity="warning" sx={{ mb: 3 }}>
                        Guardian not found. Please fill in the information below to register a new guardian.
                      </Alert>
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12}>
                          ...
                          <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                            Guardian Address (Optional - will use student's address if not provided)
                          </Typography>
                        </Grid>
          ...
          <Grid item xs={12}>
            <Box display="flex" gap={2} justifyContent="flex-end">
              ...
              <Button
                ...
                size="large"
                disabled={loading || (isChildCategory && !existingGuardian && !showGuardianForm)}
              >
